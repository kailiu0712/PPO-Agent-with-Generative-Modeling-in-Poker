...
def run_experiment(use_opponent_model=False, num_episodes=100, value_batch_size=64, policy_batch_size=100, policy_iter=10, policy_epochs=10, value_epochs=10, method='TD', lam=0.9):
    ...
    reward_hist, best_rewards_hist, win_rate = rl_agent.ppo_training(env, num_episodes=num_episodes, value_batch_size=value_batch_size, policy_batch_size=policy_batch_size,
                          ...
                          value_epochs=value_epochs, use_opponent_model=use_opponent_model, opp_model=opp_model, method=method, lam=lam)
    
    torch.save(rl_agent.policy.state_dict(), MODEL_PATH)
    print(f"[Phase 1] Saved trained model to {MODEL_PATH}")
    print(f"File size: {os.path.getsize(MODEL_PATH) / 1024:.2f} KB")

    return reward_hist, best_rewards_hist, win_rate
...
def main():
    ...
    method = "TD"
    lam = 0.9
    figname = 'stage_1_PPO_v2_stage1_1212_2'

    start_time = time.time()

    reward_hist, best_rewards_hist, win_rate = run_experiment(use_opponent_model=use_opponent_model, num_episodes=num_episodes,
    ...
    # ---- Plot reward ----
    ax1.plot(iterations, reward_hist, label="Average Reward", linewidth=2, color="steelblue")
    # ax1.plot(iterations, best_rewards_hist, label="Best Reward", linewidth=2, color="red")
    ...
    # ---- Plot win rate ----
    ax2 = ax1.twinx()
    ax2.scatter(win_iterations, win_rate, label="Win Rate", color="steelblue", s=30)
    ...
    plt.title("Reward and Win Rate during PPO Training (stage 1)")
    plt.tight_layout()
