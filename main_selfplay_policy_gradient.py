...
def run_experiment(use_opponent_model=False):
    ...
    # FROZEN opponent
    agent_frozen = RLAgent(TRUE_OBS_DIM, ACTION_SPACE, use_opponent_model=False)
    agent_frozen.policy.load_state_dict(torch.load(MODEL_PATH))
    agent_frozen.policy.eval()
    print("[Phase 2] Loaded frozen opponent model.")
    ...
    for ep in range(1, EPISODES + 1):
        ...
        for name in env.agent_iter():
            ...
            if name == "player_0":
                ...
                with torch.no_grad():
                    action, _ = agent_frozen.get_action(state, mask)
...
def main():
    ...
    plot_results(scores_no_model, scores_with_model, figname)
